...
CookieFile.prototype.addCookie = function (cookie, err = false) {
    ...
    if (!this.isCookieStorage) {
        ...
        cookie.domain &&
        cookie.domain.substr(0, 9) != "chrome://" &&
        cookie.domain.substr(0, 19) != "chrome-extension://"
    ) {
...
CookieFile.prototype.render = function () {
    ...
    this.div.getElementsByClassName("title")[0].childNodes[0].data =
        this.filename + " (" + (this.cookies.length - this.numfail) + "/" + this.cookies.length + ")";
...
function setCookie(line, cookie_file, cookie_obj, callback) {
    chrome.cookies.set(cookie_obj, function (cookie) {
        if (cookie == null || chrome.runtime.lastError != undefined) {
            cookie_file.addCookie(
                {
                    name: i18n("error") + ": chrome.cookies.set",
                    domain: i18n("line") + " " + (line + 1),
                    error2: chrome.runtime.lastError ? chrome.runtime.lastError.message : undefined,
                },
                true
            );
        } else {
    ...
    for (var i = 0, f; (f = files[i]); i++) {
        ...
        reader.onload = (function (file) {
            ...
            if (file.name.toLowerCase().includes("json")) {
                return function (e) {
                    var cookiesJson = JSON.parse(e.target.result);

                    for (var a = 0; a < cookiesJson.length; a++) {
                        var l = cookiesJson[a];
                        // Line is a comment or empty
                        var url = "http" + (l.secure == true ? "s" : "") + "://" + l.domain.replace(/^\./, "") + l.path;
                        l.url = url;
                        var cookie_obj = l;
                ...
                return function (e) {
                    ...
                    for (var a = 0; a < lines.length; a++) {
                        ...
                        if (fields.length != 7) {
                            ...
                            cookie_file.addCookie(
                                {
                                    name: i18n("error") + ": " + i18n("errorSyntax"),
                                    domain: i18n("line") + " " + (a + 1),
                                },
                                true
                            );
                            cookie_file.update("(" + parsed_lines + "/" + lines.length + ")");
        ...
        chrome.cookies.getAllCookieStores(function (stores) {
            ...
            for (var i = 0; i < stores.length; i++) {
                parseTabs(stores[i], referer_id, function () {
                    ...
                    if (
                        parsed_stores == stores.length &&
                        document.getElementById("storage-container").getAttribute("data-storage") == undefined
                    ) {
